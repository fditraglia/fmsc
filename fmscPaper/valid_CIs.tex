...
At the worst-case parameter values, for example, its median width is only 22\% greater than that of the valid estimator.
...
Various impossibility results for post-selection inference, outlined in \cite{LeebPoetscher2005}, imply that conservative intervals are the best we can do if we desire inference with uniform coverage guarantees.
This does not imply, however, that the intervals presented above are the shortest possible.
Although the simulation results presented above take $\alpha = \delta = 0.05$, note from Theorem \ref{thm:sim} that \emph{any} values of $\alpha$ and $\delta$ that sum to 0.1 yield an interval with asymptotic coverage probability no less than 90\%.

...
Although various impossibility results for post-selection inference, outlined in \cite{LeebPoetscher2005}, imply that conservative intervals are the best we can do if we desire inference with uniform coverage guarantees, the intervals presented above are the shortest possible.
...
Accordingly, as suggested by \cite{ClaeskensHjortbook} and investigated for a more general class of problems by \cite{McCloskey}, one could attempt to \emph{optimize} confidence interval width by varying $\alpha$ and $\delta$.
The primary obstacle to this idea is computational.
The simulation-based interval from Algorithm \ref{alg:conf} is quite computationally intensive when the dimension of $\tau$ is large.

...
Accordingly, Tables \ref{tab:OLSvsIVsim_width_FMSC} and \ref{tab:OLSvsIVsim_width_AVG} compare the median width of the simulation-based, two-step post-FMSC and minimum-AMSE intervals to that of the TSLS estimator in the OLS versus TSLS simulation experiment from section \ref{sec:OLSvsIVsim}.
...
In this case, the width calculation is relative to the valid estimator, the TSLS estimator includes the exogenous instruments $z_1, z_2, z_3$ but excludes the potentially endogenous instrument $w$.
...
Once the decision to carry out moment selection has been taken, however, we cannot simply ignore this fact and report the usual confidence intervals.
...
Because it can be employed to construct confidence intervals that account for the effect of such specification tests, Algorithm \ref{alg:conf} can provide a valuable robustness check in empirical work.

But is there any way to avoid the conservativeness of the simulation-based interval?
The short answer is no.
Various impossibility results for post-selection inference, outlined in \cite{LeebPoetscher2005}, imply that conservative intervals are the best we require inference with uniform coverage guarantees.
This does not imply, however, that the intervals presented above are the shortest possible.
Although the simulation results presented above take $\alpha = \delta = 0.05$, notice from Theorem \ref{thm:sim} that \emph{any} values of $\alpha$ and $\delta$ that sum to 0.1 yield an interval with asymptotic coverage probability no less than 90\%.
Accordingly, as suggested by \cite{ClaeskensHjortbook} and investigated for a more general class of problems by \cite{McCloskey}, one could attempt to \emph{optimize} confidence interval width while maintaining uniform asymptotic coverage by varying $\alpha$ and $\delta$.
The primary obstacle to this idea is computational.
The simulation-based interval from Algorithm \ref{alg:conf} is quite computationally intensive when the dimension of $\tau$ is large.\footnote{For more discussion of this point, see Section \ref{sec:application}.}
Adding another layer to the optimization procedure only makes things worse.
Developing clever computational shortcuts to surmount this problem is an interesting topic for future research.
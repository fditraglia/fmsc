...
n.reps <- 10000
rho.grid <- seq(from = 0, to = 0.25, by = 0.05)
gamma.grid <- seq(from = 0, to = 0.25, by = 0.05)
n.grid <- c(50, 100, 500)

params <- expand.grid(n = n.grid, g = gamma.grid, r = rho.grid)

CI.results <- mcmapply(CIs_compare_default_cpp, 
                     g = params$g, r = params$r, n = params$n, 
...
#Clean up
rm(n.reps, rho.grid, gamma.grid, n.grid)

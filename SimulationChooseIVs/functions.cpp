...
// [[Rcpp::export]]
List dgp_test(double g, double r, int n){
  double b = 0.5;
  colvec p = 0.1 * ones(3);
  mat Q = eye(3, 3);
  mat V(3,3); 
  ...
  dgp sim(b, p, g, V, Q, n);
  colvec e = sim.e_v_w.col(0);
  colvec v = sim.e_v_w.col(1);
  List out = List::create(sim.x, sim.y, sim.z, sim.w, e, v, V, Q, p);
  out.names() = CharacterVector::create("x", "y", "z", "w", "e", "v", "V", "Q", "p");
  return(out);

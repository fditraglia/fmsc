...
The AMSE of the OLS and 2SLS estimators takes a particularly simple form:
  \begin{eqnarray}
  \mbox{AMSE(OLS)} &=& \frac{\tau^2}{\sigma_x^4} + \frac{\sigma_\epsilon^2}{\sigma_x^2}\\
  \mbox{AMSE(2SLS)} &=& \frac{\sigma_\epsilon^2}{\gamma^2}
  \end{eqnarray}
where $\sigma_x^2 = \gamma^2 + \sigma_v^2$ and $\gamma^2 = \boldsymbol{\pi}'Q_z \boldsymbol{\pi}$. The AMSE of the 2SLS estimator is lower than that of the OLS estimator when
  \begin{eqnarray*}
    ...
    \tau^2 &>&\sigma_\epsilon^2\sigma_x^2\left(\frac{\sigma_x^2}{\gamma^2} - 1\right)\\
        ...
        \tau^2  &>& \sigma_x^2 \sigma_\epsilon^2\left(\frac{\sigma_v^2}{\gamma^2}\right)\\
              ...
              \tau^2  &>& \sigma_v^2 \sigma_\epsilon^2\left(\frac{\gamma^2 + \sigma_v^2}{\gamma^2}\right)\\
\tau^2  &>& \sigma_v^2 \sigma_\epsilon^2\left(1 + \frac{\sigma_v^2}{\boldsymbol{\pi}'Q_z \boldsymbol{\pi}}\right)\\
\left|\tau \right| &>& \sigma_v \sigma_\epsilon \sqrt{\left(1 + \frac{\sigma_v^2}{\boldsymbol{\pi}'Q_z \boldsymbol{\pi}}\right)}\\
\left|\tau \right| &>& \sigma_v \sigma_\epsilon \sqrt{1 + \frac{1}{\kappa^2}}
  \end{eqnarray*}
Where $\kappa^2 = \boldsymbol{\pi}'Q_z \boldsymbol{\pi}/\sigma_v^2$ is the ``signal-to-noise'' ratio for the instruments. We see that the FMSC trades the endogeneity of $x$, as measured by $\tau$, against the strength of the instruments, as measured by the concentration parameter $\kappa^2$. 

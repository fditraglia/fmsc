...
On the whole, the simulation-based intervals constructed using Algorithm \ref{alg:conf} work well, with two caveats.
...
Second, and more importantly, moment selection comes with a cost: the simulation-based intervals are on average wider than a textbook confidence interval for the valid estimator.
Given this fact, one might wonder whether it would be better to simply use the valid estimator and report a traditional confidence interval.

Although there is no definitive answer to this question, recall that the primary goal of the the FMSC is \emph{estimation} rather than inference.

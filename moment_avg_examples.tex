...
\todo[inline]{In some settings can construct averaging estimator that uniformly dominates ``conservative'' estimator (Xu and Bruce) but I'm working in a setting with a scalar target parameter.}
...
In at least some applications, however, there is a compelling alternative to the exponential weighting scheme: one can instead derive weights \emph{analytically} to minimize AMSE within the FMSC framework.\footnote{I thank Bruce Hansen for suggesting this idea.} 
...
Let $\widetilde{\beta}(\omega)$ be a convex combination of the OLS and TSLS estimators from Equations \ref{eq:OLS} and \ref{eq:TSLS}, namely  
\begin{equation}
...
\end{equation}
This expression employs the same consistent estimators of $\sigma_x^2, \gamma$ and $\sigma_{\epsilon}$ as the FMSC expressions from Section \ref{sec:FMSCforOLSvsIV}.

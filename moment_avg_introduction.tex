...
The point is not that conservative criteria are immune to the effects of selection on inference: it is that conservative criteria can be studied using asymptotics that more accurately represent the phenomena encountered in finite samples.

There are two main problems with traditional confidence intervals na\"{i}ely applied post-moment selection.
...
Accordingly, because they \emph{condition} on the selected moment conditions, traditional intervals are too short.
Second, traditional confidence intervals ignore the fact that selection is carried out over \emph{potentially invalid} moment conditions.
...
The key point is that the post-selection estimator is a \emph{randomly-weighted average} of the individual candiate estimators.

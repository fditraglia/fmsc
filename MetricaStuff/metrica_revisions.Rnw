...
<<OLSboundary, dev = "tikz", echo=FALSE, cache=TRUE>>=
boundary.plot <- function(n, xlabel = "", ylabel = ""){
  x <- seq(from = 0.001, to = 0.999, by = 0.001)
  y <- sqrt((1/n) * (1/x))
  plot(x,y, type = 'l', xlim = c(0,1), ylim = c(0,1) ,xaxs = "i", yaxs = "i", pty = "s", xlab = xlabel, ylab = ylabel)
  
  poly <- cbind(c(0, x, 1), c(0, y, 0))
  polygon(poly, density = 20)
          
  text(0.5, 0.8, cex = 1.75, bquote(n == .(n)))
}

par(mfrow = c(2,2))

boundary.plot(10, ylabel = "Absolute Correlation")
boundary.plot(100)
boundary.plot(500, ylabel = "Absolute Correlation", xlabel = "R-squared")
boundary.plot(1000, xlabel = "R-squared")
@

...
boundary.plot <- function(n, xlabel = "", ylabel = ""){
  x.max <- 2
  x <- seq(from = 0.001, to = x.max, by = 0.001)
  y <- sqrt((1/n) * (1 + 1/x))
  plot(x,y, type = 'l', xlim = c(0,x.max), ylim = c(0,1) ,xaxs = "i", yaxs = "i", pty = "s", xlab = xlabel, ylab = ylabel)
  
  poly <- cbind(c(0, x, x.max), c(0, y, 0))
  polygon(poly, density = 20)
          
  text(x.max/2, 0.8, cex = 1.5, bquote("$n$" == .(n)))
}

par(mfrow = c(2,2), mgp=c(2.2,0.45,0), tcl=-0.4, mar=c(3.3,3.6,1.1,1.1))

boundary.plot(10, ylabel = "$|\\rho_{\\epsilon v}|$")
boundary.plot(100)
boundary.plot(500, ylabel = "$|\\rho_{\\epsilon v}|$", xlabel = "$\\kappa^2$")
boundary.plot(1000, xlabel = "$\\kappa^2$")
@

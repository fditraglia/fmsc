...
$$\widehat{\beta}_S = (X'Z_S W Z_S' X)^{-1}X'Z_S W_S Z_S' \mathbf{y}$$
where $S$ indexes the instrument set under consideration and we always include $Z_1$ in the instrument set. Expanding,
  \begin{eqnarray*}
    \widehat{\beta}_S &=& (X'Z_S W Z_S' X)^{-1}X'Z_S W_S Z_S' \mathbf{y}\\
      &=& (X'Z_S W Z_S' X)^{-1}X'Z_S W_S Z_S'(X\beta + \boldsymbol{\epsilon})\\
      &=& \beta +(X'Z_S W Z_S' X)^{-1}X'Z_S W_S Z_S'\boldsymbol{\epsilon}
  \end{eqnarray*}
and hence, 
  $$\sqrt{n}(\widehat{\beta}_S -\beta) = \sqrt{n}(X'Z_S W Z_S' X)^{-1}X'Z_S W_S Z_S'\boldsymbol{\epsilon}$$
  For the special case of two-stage least squares (2SLS), we have $W_S = (Z_S' Z_S)^{-1}$ and hence
  \begin{eqnarray*}
  ...
  \end{eqnarray*}
where $P_S = Z_S(Z_S'Z_S)^{-1} Z_S'$, $\widetilde{Z} = P_S Z$, $\widetilde{\mathbf{v}} = P_S \mathbf{v}$ and we have used the fact that $P_S$ is symmetric and idempotent.

\begin{eqnarray*}
  (\Pi'\widetilde{Z}' + \widetilde{\mathbf{v}}')(\widetilde{Z} \Pi + \widetilde{\mathbf{v}}) = \Pi' \widetilde{Z}' \widetilde{Z} \Pi
\end{eqnarray*}

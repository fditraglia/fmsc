...
since both $\left(\Sigma_{yx} \textbf{a}\textbf{a}' \Sigma_{xy} \right)$ and $\Sigma_{yy}$ are symmetric matrices. Rearranging and left-multiplying by $\Sigma_{yy}^{-1}$ gives 
  $$\left(\Sigma_{yy}^{-1}\Sigma_{yx} \textbf{a}\textbf{a}' \Sigma_{xy} - \lambda_b \textbf{I}\right)\textbf{b} = \textbf{0}$$
Hence, $\lambda_b$ and $\mathbf{b}$ are an \emph{eigenvalue-eigenvector pair} of the matrix $\Sigma_{yy}^{-1}\Sigma_{yx} \textbf{a}\textbf{a}' \Sigma_{xy}$. 
Note that, since it is simply the outer-product of the vector $\mathbf{a}$, the matrix $\mathbf{a}\mathbf{a}'$ has rank one.\footnote{Its $j$th column is simply $a_j \mathbf{a}$, where $a_j$ is the $j$th element of $\mathbf{a}$.} 

...
To simplify the notation in this section, define the following selection matrices corresponding to $Z_1$ and $Z_2$: 
  \begin{eqnarray*}
  \Xi_1 &=& \left[\begin{array}{cc} I_{q_1} & 0_{q_2 \times q_2}  \end{array}\right]\\
...
\begin{eqnarray*}
  \widehat{\tau} &\overset{d}{\rightarrow}& \boldsymbol{\tau} + \Xi_2 M - \Xi_2 Q_Z \Pi K_1 \Pi \Xi_1 M\\
    &=& \boldsymbol{\tau} + \Xi_2(M - Q_Z \Pi K_1 \Xi_1 M)\\
    &=& \boldsymbol{\tau} + \Xi_2(I_q - Q_Z \Pi K_1 \Xi_1) M
\end{eqnarray*}
Since all the limit results we have written down depend only on $M$ and constant matrices, we in fact have \emph{joint convergence} of any collection of $\sqrt{n}\left(\widehat{\beta}_S - \beta \right)$ estimators \emph{along} with $\widehat{\tau}$. We will use this fact to construct valid confidence intervals.
...
It follows immediately that 
  ...
  \end{eqnarray*}
We estimate $\nabla\mu_\beta(\beta)$ by $\nabla_\beta\mu(\widehat{\beta}_1)$ when necessary (sometimes the derivatives don't depend on $\beta$), and $K_S$ by $\widehat{K}_S$. Both of these will be consistent. Various estimators of $\Omega$ are possible, but the point is that we can easily get a consistent estimator under these asymptotics. The only problem is $\tau \tau'$. We can construct an asymptotically unbiased estimator by using the fact that 


\todo[inline]{Target parameter, delta method, expression for estimate of AMSE. Asymptotically unbiased estimator of asymptotic bias. Write out the special case when the target parameter is a linear combination of the elements of $\beta$. This includes as an even more special case situation in which we're only interested in one of the parameters.}
...
\todo[inline]{Add in the simplification of the variance matrix that results from the assumption that there is no heteroscedasticity in the limit.}

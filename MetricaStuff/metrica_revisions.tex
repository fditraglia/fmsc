...
To estimate $\tau$ we need an an asymptotically unbiased estimator of $\beta$ to plug into the moment conditions for the ``suspect'' instruments contained in $Z_2$. Let $\widehat{\beta}_V$, for ``valid,'' denote the GIV estimator based \emph{only} on the instruments contained in $Z_1$:
\begin{equation}
  \widehat{\beta}_V = \left(X'Z_1 W^V_n Z_1' X\right)^{-1} X'Z_1 W_n^V Z_1' \mathbf{y}
\end{equation}
By the result in the previous section,
  \begin{eqnarray*}
    \sqrt{n} \left(\widehat{\beta}_V - \beta \right) &\overset{d}{\rightarrow}& K_V \left[\begin{array}{cc} I_{q_1} & 0_{q_2 \times q_2}  \end{array}\right] \left(\left[\begin{array}
           {c} \mathbf{0} \\ \boldsymbol{\tau}
         \end{array}\right] + M \right)\\
          &\sim& K_V  \times N\left(0, \lim_{n\rightarrow \infty} E_n[\epsilon_i^2 \mathbf{z}_{i1}\mathbf{z}_{i1}']\right)
  \end{eqnarray*}

We estimate $\tau$ as follows:
\begin{eqnarray*}
  \widehat{\tau} &=& \sqrt{n} \left[ Z_2' \left(\textbf{y} - X\widehat{\beta}_V\right)/n\right] = n^{-1/2} Z_2'\left(\textbf{y} - X\widehat{\beta}_V\right)\\
      &=& n^{-1/2} Z_2'(X\beta + \boldsymbol{\epsilon} - X \widehat{\beta}_V) = n^{-1/2} Z_2' \left[\boldsymbol{\epsilon} - X(\widehat{\beta}_V - \beta)\right]\\
      &=& n^{-1/2} Z_2'\boldsymbol{\epsilon} - n^{-1/2} Z_2' X (\widehat{\beta}_V - \beta)\\
      &=& n^{-1/2} Z_2'\boldsymbol{\epsilon} - (Z_2' X/n) \left[  \sqrt{n}(\widehat{\beta}_V - \beta)\right]
\end{eqnarray*}
We have
  \begin{eqnarray*}
    Z_2'X/n &=& n^{-1} Z_2'(Z\Pi + \mathbf{v}) = n^{-1}Z_2'Z\Pi + n^{-1}Z_2'\mathbf{v}\\  
            &=& n^{-1}Z_2' \left[\begin{array}
              {cc} Z_1 & Z_2 
            \end{array} \right] + n^{-1}Z_2'\mathbf{v}\\
            &\overset{p}{\rightarrow}& 
  \end{eqnarray*}
  \todo[inline]{Need to express as a matrix product in terms of $Q_Z$.}
since $n^{-1}Z_2'\mathbf{v} \overset{p}{\rightarrow} 0$.

...
\subsection{Valid Confidence Interval for OLS vs 2SLS Example} % (fold)
\label{sub:valid_confidence_interval_for_ols_vs_2sls_example}

\todo[inline]{Bootstrap the limit experiment using the asymptotic normality of the estimators. Start with the situation where we assume $\tau$ is known. Then take the supremum over all possible $\tau$ within a particular confidence interval.}

There doesn't seem to be a shortcut that makes this any cleaner, so we'll just have to write it out from first principles. The post-FMSC estimator is
...
\todo[inline]{We need to substitute estimators. Difficulty with handling $\tau^2$ since we only have an asymptotically unbiased estimator of this. Can't get an asymptotically unbiased estimator of the optimal weight in this case. I don't want to get side-tracked on this so perhaps just try the simplest thing: substitute the asymptotically unbiased estimator of $\tau^2$. This will be a short section in any case and is intended to be illustrative more than anything else.}

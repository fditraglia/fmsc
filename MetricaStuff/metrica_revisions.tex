...
Using this convention, $Z_1 = Z\Xi_1'$ and similarly $Z_2 = Z \Xi_2'$
...
\begin{equation}
  \widehat{\beta}_1 = \left(X'Z_1 W^1_n Z_1' X\right)^{-1} X'Z_1 W_n^1 Z_1' \mathbf{y}
\end{equation}
...
\begin{eqnarray*}
  \widehat{\tau} &=& \sqrt{n} \left[ Z_2' \left(\textbf{y} - X\widehat{\beta}_1\right)/n\right] = n^{-1/2} Z_2'\left(\textbf{y} - X\widehat{\beta}_1\right)\\
      &=& n^{-1/2} Z_2'(X\beta + \boldsymbol{\epsilon} - X \widehat{\beta}_1) = n^{-1/2} Z_2' \left[\boldsymbol{\epsilon} - X(\widehat{\beta}_1 - \beta)\right]\\
      ...
      &=&n^{-1/2} \Xi_2 Z'\boldsymbol{\epsilon} - (Z_2' X/n)\left( -\widehat{K}_1 \Xi_1 n^{-1/2}Z'\boldsymbol{\epsilon} \right)\\
      &=& \left[\Xi_2 + \left(\frac{-Z_2'X}{n}\right)\left(-\widehat{K}_1\right)\Xi_1\right](n^{-1/2}Z'\boldsymbol{\epsilon})\\
      &=& \left[\left(\frac{-Z_2'X}{n}\right)\left(-\widehat{K}_1\right)\Xi_1 + \Xi_2 \right](n^{-1/2}Z'\boldsymbol{\epsilon})\\
      &=& \left\{\left(\frac{-Z_2'X}{n}\right)\left(-\widehat{K}_1\right)\left[\begin{array}{cc} I_{q_1} & 0_{q_1 \times q_2}  \end{array}\right] + \left[ \begin{array}{cc}
              0_{q_2 \times q_1}& I_{q_2}
            \end{array}\right] \right\}(n^{-1/2}Z'\boldsymbol{\epsilon})\\
      &=&\left\{\left[ \begin{array}
        {cc} -n^{-1}Z_2'X \left( -\widehat{K}_1 \right)& 0_{q_2 \times q_2} 
      \end{array} \right] +\left[ \begin{array}{cc}
              0_{q_2 \times q_1}& I_{q_2}
            \end{array}\right]\right\}(n^{-1/2}Z'\boldsymbol{\epsilon})\\
      &=& \left[ 
     \begin{array}
        {cc}  -n^{-1}Z_2'X \left( -\widehat{K}_1 \right) & I_{q_2}
      \end{array} 
       \right] (n^{-1/2}Z'\boldsymbol{\epsilon})
\end{eqnarray*}
Noting that $Z_2'X(-\widehat{K}_1)$ is $q_2\times q_1$ since $Z'$ is $q_2\times n$, $X$ is $n\times p$ and $\widehat{K}_1$ is $p\times q_1$.
We can read off our estimator $\widehat{\Psi}$ of $\Psi$ from the final equality above, namely
\begin{eqnarray*}
  \widehat{\Psi} &=& -n^{-1}Z_2'X \left( -\widehat{K}_1 \right)\\
    &=& -n^{-1}Z_2'X \left[ n(X'Z_1 W^1_n Z_1' X)^{-1}X'Z_1 W^1_n \right]  \\
    &=&-Z_2'X(X'Z_1 W^1_n Z_1' X)^{-1}X'Z_1 W^1_n\\
    &=&n^{-1}Z_2'X \widehat{K}_1 
\end{eqnarray*}
\textcolor{red}{Note the sign convention here!} This comes from the fact that we defined
\[
-\widehat{K}_1 = n(X'Z_1 W^1_n Z_1' X)^{-1}X'Z_1 W^1_n
\]
for consistency with the notation for the generic GMM version of the problem.
...
\textcolor{red}{Be careful about $\Psi$! In particular, see the explanation about our sign convention described in Section \ref{sec:est_tau}.}
...
by the shortcut formula. We can continue to estimate $E[\epsilon_i^2 \textbf{z}_i \textbf{z}_i']$ by $Z'DZ/n$, or in the case of homoscedasticity by $\widehat{\sigma}_\epsilon^2 Z'Z/n$, so the question is how to estimate the second term. In the presence of over-identifying restrictions, the moment conditions used to estimate the model will not hold exactly in-sample. This is the principle behind the $J$-test. We can use the same idea here to estimate $E[\epsilon_i \textbf{z}_i]$ by the sample analogue $n^{-1} \sum_{i=1}^n \widehat{\epsilon}_i \textbf{z}_i$. This idea leads to the heteroskedasticity-consistent centered variance matrix estimator:

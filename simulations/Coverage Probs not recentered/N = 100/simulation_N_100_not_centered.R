...
#This is basically the same as the September 23rd Version. The only difference is that now we do not recenter the confidence intervals of post-selection estimators to provide a better comparison with the corrected intervals simulated elsewhere.

#----------------------------------------------------------#
#                    CONSTANT MACROS                       #
#----------------------------------------------------------#

N <- 100 #sample size of each simulation
replications <- 10000 #Number of replications
outfilename <- "sim_results_100_not_centered.csv"
output.dir <- './N = 100'
...
#For running simulations from the F drive
#root.dir <- "F:/Coverage Probs not recentered"

#Updated October 6th, 2011 to give the option of not re-centering confidence intervals around the quasi-true (biased) value when computing coverage probabilities. The default is not to re-center.
...
simulation.study <- function(N.reps, sample.size, relevance.w, endog.w, center.coverage = FALSE){
    ...
    #Coverage of full and valid models, recentered around quasi-true values
  ...
  #If we really should be including w based on the finite-sample RMSE given the current values of relevance.w and endog.w, then the correct decision rate is simply the frequency with which w is included by a given criterion. Otherwise it is the frequency with which w is excluded.
    ...
    summarize.criterion <- function(criterion.includes.w){
      ...
      #Coverage for naive confidence interval around the actual true value
      COVERAGE <- coverage(estimator = b.criterion, se = SE.criterion, true.value = b.TRUE)

...
compute.criteria.naive <- function(X, y, Z1, Z2, B = 1000, alpha = 0.05){
  ...
  #---------------------------------------------#
	#-----NAIVE CONFIDENCE INTERVAL FOR FMSC------#
	#---------------------------------------------#
  ...
  lower.naive <- mu.hat - (upper.lambda)/sqrt(N)
  upper.naive <- mu.hat - (lower.lambda)/sqrt(N)
  ...
  #Need to return the FMSC so that we can calculate the bias of the procedure which we'll need in the step where we see if the coverage is correct.
	out <- data.frame(b, b.SE, FMSC, b1, b1.SE, FMSC.1, lower.naive, upper.naive, row.names = NULL)

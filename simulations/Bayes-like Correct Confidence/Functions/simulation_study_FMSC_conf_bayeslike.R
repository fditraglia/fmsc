...
simulation.study.FMSC.conf.bayeslike <- function(N.reps, sample.size, relevance.w, endog.w){
	
	#Initialize data.frame to hold results
	results <- matrix(NA, nrow = N.reps, ncol = 8)
	results <- as.data.frame(results)
	names(results) <- c('b', 'b.SE', 'FMSC', 'b1', 'b1.SE', 'FMSC.1', 'lower.bayeslike', 'upper.bayeslike')
	...
	for(i in  1:N.reps){
		...
		temp.results <- compute.criteria.bayeslike(X = sim$X, y = sim$y, Z1 = sim$Z1, Z2 = sim$Z2)
 ...
 FMSC.mean <- mean(b.FMSC)
  ...
  contains.truth <- (lower.bayeslike <= b.TRUE) & (b.TRUE <= upper.bayeslike) #elementwise AND

 bayeslike.correct.cover <- sum(contains.truth)/length(contains.truth)
 
 detach(results)
 
 out <- data.frame(relevance.w, endog.w, bayeslike.correct.cover)
 names(out) <- c('relevance.w', 'endog.w', 'bayeslike.FMSC.correct.cover')
 
 return(out)
	
	
}#END simulation.study.FMSC.conf.Bayeslike
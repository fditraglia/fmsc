...
When this is not the case, it remains possible to use the AMSE framework to carry out a sensitivity analysis.\footnote{See Appendix \ref{sec:digress}.}
\todo[inline]{Mention the uniformity stuff: because I study a scalar target parameter, can't get uniform improvements but can do better in large region of the parameter space. Particularly concerned with settting where you think extra assumptions may be nearly correct}
...
Using off-the-shelf optimization routines, this procedure is well within the ability of the average desktop computer for problems of realistic scale.
...
Although consistency is a crucial minimal property in many settings, the situation is more complex for model and moment selection: consistent and conservative selection procedures have different strengths, but these strengths cannot be combined \cite{Yang2005}.
The motivation behind the FMSC, however, is minimum-risk estimation.
...
An idea similar to that which motivates the FMSC underlies the combined moments (CM) estimator of \cite{Judge2007}, who emphasize that incorporating the information from an incorrect specification could lead to favorable bias-variance tradeoff. 
...
\todo[inline]{Also cite Xu's paper and try to explain how what I'm doing is different! May also want to say something about MSE vs other risk functions and existence etc.}
...
Extending the idea behind the FMSC to a more general framework that allows for weak identification or a growing number of moment conditions is a challenging topic that I leave for future research.
...
Section \ref{sec:FMSC} derives the FMSC and presents specialized results for the two running examples: OLS versus TSLS and choosing instrumental variables.
...
Proofs and computational details appear in the Appendix along with supplementary material.

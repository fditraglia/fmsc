...
#This uses a quasi-Bayes two-stage procedure
#This one has a "plus" rather than minus to try to get a handle on why the minus term works 
...
outfilename <- "sim_results_FMSC_bayeslike_correct_N_500.csv"
output.dir <- './Test Run'
...
root.dir <- "C:/Documents and Settings/rt356/My Documents/My Dropbox/PhD Dissertation/Job Market Paper/Simulations/Bayes-like Plus"
...
source("reg2SLS.R")
source("correct_FMSC_interval_bayeslike.R")
source("linear_simulation.R")
source("simulation_study_FMSC_conf_bayeslike.R")
library(mvtnorm)
...
names(results) <- c('relevance.w', 'endog.w', 'bayeslike.FMSC.correct.cover')
...
for(i in 1:nrow(parameters)){
  ...
  results[i,] <- simulation.study.FMSC.conf.bayeslike(N.reps = replications, sample.size = N, relevance.w = parameters$g[i], endog.w = parameters$rho[i])

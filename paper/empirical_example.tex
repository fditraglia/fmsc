...
Based on the arguments given in \cite{Acemoglu} and \cite{Sachs}, I proceed under the assumption that ln\emph{mort} and \emph{maleco}, measures of early settler mortality and malaria ecology, are exogenous.
Rather than selecting over all 128 possible instrument sets, I consider eight specifications formed from the four sets of instrument ``blocks'' defined by \cite{Carstensen2006}.
...
In each, the two best instrument sets are numbers 5 (baseline, climate and Europe) and 8 (all instruments).\footnote{In Table 2 of their paper, \cite{Carstensen2006} report GMM-BIC and HQ results for selection over instrument sets 2--4 and 8 that favor instrument set 8. They do not consider instrument sets 5--7.}
...
These are constructed by simply identifying the selected instrument set from Table \ref{tab:FMSC_values} and reporting the corresponding nominal 95\% interval from Table \ref{tab:fullresults}.
...
Rather than taking the minimum lower confidence limit and maximum upper confidence limit over all values in a given confidence region for $\tau$, this procedure simply assumes that the estimated value $\widehat{\tau}$ equals the true value.
Neither the Na\"{i}ve nor the 1-Step procedures yield valid 95\% confidence intervals.
They are provided merely for comparison with the 2-Step procedure, which fully implements Algorithm \ref{alg:conf} with $\alpha = \delta = 0.05$ and $J=10,000$. As explained above, the 2-Step interval is guaranteed to have asymptotic coverage probability of at least $1 - \alpha - \delta$ , in this case 90\%.
...
Given the simplicity of the second-stage regression and the relatively small sample size, one might reasonably wonder whether this empirical example represents a realistic proof of concept for FMSC instrument selection and post-selection inference in real applications.
The answer is yes: this example was selected precisely because it is as simple as possible in those respects that matter \emph{least} for the FMSC and confidence interval calculations yet fully realistic in those respects that matter most.
The computational complexity of the procedures described above is determined almost entirely by the dimension, $q$, of $\tau$, in this case the number of suspect instruments in the full instrument: seven.
This is because the 2-Step confidence interval procedure requires us to carry out two seven-dimensional constrained optimization problems with a stochastic objective function: one for the lower confidence limit and another for the upper confidence limit.
...
By comparison, the number of instrument sets under consideration is far less important

something about being able to pre-compute

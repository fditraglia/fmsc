...
\todo[inline]{Consider iid setting.}

Consider a linear instrumental variables (IV) model of the form
\begin{eqnarray}
    y_i &=& \mathbf{x}_i' \beta +  \epsilon_i\\
    \mathbf{x}_i &=&  \Pi_1' \mathbf{z}_{i}^{(1)} + \Pi_2'\mathbf{z}_{i}^{(2)} + \mathbf{v}_i
\end{eqnarray}
where $y$ is an outcome of interest, $\mathbf{x}$ is a $p$-vector of regressors, some of which are endogenous, $\mathbf{z}^{(1)}$ is a $q_1$-vector of instruments known to be exogenous, and $\mathbf{z}^{(2)}$ is a $q_2$-vector  of \emph{potentially endogenous} instruments. The $p$-vector $\beta$, $q_1\times p$ matrix $\Pi_1$, and $q_2\times p$ matrix $\Pi_2$ contain unknown constants. Stacking observations in the usual way, let $\mathbf{y}' = (y_1, \hdots, y_n)$, $X' = (\mathbf{x}_1, \hdots, \mathbf{x}_n)$, $\boldsymbol{\epsilon} = (\epsilon_1, \hdots, \epsilon_n)$, 
$Z_1' = (\mathbf{z}_{1}^{(1)}, \hdots, \mathbf{z}_{n}^{(1)})$, $Z_2' = (\mathbf{z}_{1}^{(2)}, \hdots, \mathbf{z}_{n}^{(2)})$, and $V' = (\mathbf{v}_1, \hdots, \mathbf{v}_n)$ where $n$ is the sample size. Using this notation, $\mathbf{y} = X\beta +\boldsymbol{\epsilon}$ and $X =  Z \Pi + V$, where $Z = (Z_1, Z_2)$ and $\Pi = (\Pi_1', \Pi_2')'$. 

The question is whether to use any of the instruments contained in $Z_2$ in estimation. Tradeoff between plausibility of exclusion restriction and desire for strong instruments to reduce variance. 

General linear GMM estimator:
$$\widehat{\beta}_S = (X'Z_S \widetilde{W}_S Z_S' X)^{-1}X'Z_S \widetilde{W}_S  Z_S' \mathbf{y}$$
where $S$ indexes which, if any, of the instruments in $Z_2$ are used in estimation, $Z_S'  = \Xi_S Z'$, $|S|$ is the number of instruments and $\widetilde{W}_S$ is a weighting matrix. In this example we always include the instruments in $Z_1$. In this example, the local mis-specification assumption is given by
\begin{equation}
  E\left[\begin{array}
    {c}
...
\right]
\end{equation}
where $\mathbf{0}$ is a $q_1$-vector of zeros, and $\boldsymbol{\tau}$ is a $q_2$-vector of unknown constants.



\subsection{Example: OLS versus 2SLS}
\label{sec:OLSvsIVlowlevel}
The preceding example had a simple, nested structure: it amounted to choosing \emph{overidentifying restrictions}. The FMSC framework, however, does not rely upon this structure. To illustrate how it can be used to choose between non-nested moment sets and and estimators that may be just identified, we consider a familiar example: choosing between 
ordinary least squares (OLS) and two-stage least squares (2SLS) estimators. To keep the presentation simple, suppose that our goal is to estimate the effect $\beta$ of a single endogenous regressor $x$ on an outcome of interest $y$ in an iid, homoskedastic setting.

\todo[inline]{Admittedly this is a very simple example, but it will permit us to learn a lot about how the FMSC works.}
...
$\mathbf{z}' = (z_{1}, \hdots, z_{n})$, $Z' = (\mathbf{z}_{1}, \hdots, \mathbf{z}_{n})$, $\mathbf{x}' = (x_{1}, \hdots, x_{n})$ and so on. The two estimators under consideration are the OLS estimator $\widehat{\beta}_{OLS}$ and the 2SLS estimator $\widetilde{\beta}_{2SLS}$
  \begin{eqnarray} 
	...
	\end{eqnarray}
where $P_Z = Z(Z'Z)^{-1}Z'$. But why should we even \emph{consider} using OLS if $x$ is endogenous and we have valid instruments at our disposal? The answer is simple: 2SLS is a high variance estimator. Depending on the degree of endogeneity present in $x$ and the strength of the instruments, $\widehat{\beta}_{OLS}$ could easily have the lower mean-squared error of the two estimators.\footnote{Because the moments of the 2SLS estimator only exist up to the order of overidentificiation \citep{Phillips1980} this statement should be understood to refer to ``trimmed'' mean-squared error when the number of instruments is two or less. For more discussion of this point, see the simulation results below in section ??. \todo[inline]{Insert crossref!}} We can embed this intuition within the local mis-specification framework as follows
  \begin{equation}
...
\begin{assump}[OLS versus 2SLS]\label{assump:OLSvsIV}
	...
	\begin{enumerate}[(a)]
		...
		\item $E[\mathbf{z}_{ni} \epsilon_{ni}]=\mathbf{0}$, and $E[\epsilon_{ni}v_{ni}] = \tau/\sqrt{n}$ for all $n$
		...
		\item $E[\mathbf{z}_{ni} \mathbf{z}_{ni}'] \rightarrow Q_z>0$, $E[v_{ni}^2]\rightarrow \sigma_v^2 >0$, and $E[\epsilon_{ni}^2] \rightarrow \sigma_\epsilon^2 >0$ as $n\rightarrow \infty$
		\item $\left(E[v_{ni}\epsilon_{ni}^2] - E[v_{ni}]E[\epsilon_{ni}^2]\right) \rightarrow 0$ and $\left(E[v_{ni}^2 \epsilon_{ni}^2] - E[v_{ni}^2]E[\epsilon_{ni}^2]\right)\rightarrow 0$ as $n\rightarrow \infty$
		\item $x_{ni} = \mathbf{z}_{ni}'\boldsymbol{\pi} + v_i$ where $\boldsymbol{\pi} \neq \mathbf{0}$, and $y_{ni} = \beta x_{ni} + \epsilon_{ni}$
	\end{enumerate}
\end{assump}

\todo[inline]{Comment on the assumptions. Basically standard. Which ones are for the CLT? Assumption (e) is the homoskedasticity assumption: it only places an asymptotic restriction. Note that when the errors are normally distributed, this follows immediately from Isserli's Theorem. See Wiki. Proof of following Theorem is standard and hence omitted. Just substitute DGP, use Lindeberg-Feller, and $L_2$ WLLN. Only step worth commenting on is checking the Lindeberg condition.}
...
\sigma_\epsilon^2 \left[ \begin{array}{cc}
  ...
  \right)
$$
where $\sigma_x^2 = \gamma^2 + \sigma_v^2$, $\gamma^2 = \boldsymbol{\pi}'Q_z \boldsymbol{\pi}$, and $Q_z$, $ \sigma_\epsilon^2$, and $\sigma_v^2$ are defined in Assumption \ref{assump:OLSvsIV}.
\end{thm}
\todo[inline]{Add a proof sketch to the appendix.}
We see from the preceding result that the variance of the OLS estimator is always strictly lower than that of the 2SLS estimator since $\sigma^2_\epsilon/\sigma_x^2 = \sigma^2_\epsilon/(\gamma^2 + \sigma_v^2)$. Unless $\tau = 0$, however, OLS shows an asymptotic bias. In contrast, the 2SLS estimator is asymptotically unbiased regardless of the value of $\tau$. 
...
where $M_S = \Xi_S M$, $\tau_S = \Xi_S (0', \tau')'$ and $M$ is defined in Theorem \ref{thm:normality}. Hence,
...
where $K_v$ is defined in Corollary \ref{cor:valid}. Thus, $\widehat{\tau}\rightarrow_d (\Psi M + \tau) \sim N_q(\tau, \Psi \Omega \Psi')$.
\end{thm}
...
To estimate $\sigma_\epsilon^2$ we have two choices: we can either use the residuals from the OLS estimator or those from the 2SLS estimator. Under local mis-specification, both provide a consistent estimator of $\sigma_\epsilon^2$. We would expect the estimator based on the 2SLS residuals to be more robust, however, unless the instruments are quite weak. This is because the exogeneity of $x$, even though it disappears in the limit under our asymptotics, is non-zero in finite samples. Thus, we use 
...
to estimate $\sigma_\epsilon^2$ below. All that remains is to estimate $\tau^2$. Specializing Theorem \ref{thm:tau} and Corollary \ref{cor:tautau} to the present example gives the following result.
\begin{thm}
...
where $\widehat{V} = \widehat{\sigma}_v^2 \widehat{\sigma}_\epsilon^2 \widehat{\sigma}_x^2/\widehat{\gamma}^2$. The quantity $\widehat{T}_{FMSC}$ looks very much like a test statistic and indeed it can be viewed as such. By Theorem \ref{thm:tauOLSvsIV} and the continuous mapping theorem, $\widehat{T}_{FMSC} \rightarrow_d \chi^2(1)$. This means that we can interpret the FMSC as a test of the null hypothesis $H_0\colon \tau = 0$ against the two-sided alternative with a critical value of $2$. This corresponds to a significance level of $\alpha \approx 0.16$. 

But how does this novel ``test'' compare to something more familiar, say the Durbin-Hausman-Wu (DHW) test? It turns out that in this particular example, although not in general, carrying out moment selection via the FMSC is \emph{numerically equivalent} to using OLS unless the DHW test rejects at the 16\% level. In other words, $\widehat{T}_{FMSC} = \widehat{T}_{DHW}$. To see why this is so first note that 
...
converges in distribution to a $\chi^2(1)$ random variable. Now, rewriting $\widehat{V}$, we find that
...
using the fact that $\widehat{\sigma}_v = \widehat{\sigma}_x^2 - \widehat{\gamma}^2$. Thus, to show that $\widehat{T}_{FMSC} = \widehat{T}_{DHW}$, all that remains is to establish that $\widehat{\tau}^2 = n\widehat{\sigma}_x^4 (\widehat{\beta}_{OLS} - \widetilde{\beta}_{2SLS})^2$, which we obtain as follows:
...
\todo[inline]{Need to correct notation from here down. Also correct the notation in the associated proofs.}

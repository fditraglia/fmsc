...
The question is whether to use any of the instruments contained in $Z_2$ in estimation. Tradeoff between plausibility of exclusion restriction and desire for strong instruments to reduce variance. In this example we will always include the instruments in $Z_1$ so selection is only carried out over $Z_2$.
...
\begin{thm}[Choosing IVs Limit Distribution] Suppose that $\widetilde{W}_S \rightarrow_p W_S >0$. Then, under Assumption \ref{assump:chooseIV}
 $$\sqrt{n}\left(\widehat{\beta}_S - \beta \right) \overset{d}{\rightarrow} -K_S \Xi_S \left(\left[\begin{array}
  ...
  where $Q_S = Q_Z \Xi_S'$
      ...
      \end{eqnarray*}
and $Q, \Omega$ are defined in Assumption \ref{assump:chooseIV}.
\end{thm}

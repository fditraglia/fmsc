...
$$\mbox{AMSE(OLS)} = \frac{\tau^2}{\sigma_x^4} + \frac{\sigma_\epsilon^2}{\sigma_x^2},\quad \quad
 ...
 To implement this expression, we need to estimate the unknowns. The familiar estimators of $\sigma_x^2, \gamma^2$, and $\sigma_v^2$ remain consistent under Assumption \ref{assump:OLSvsIV}
 \begin{eqnarray*}
  	\widehat{\sigma}_x^2 &=& n^{-1}\mathbf{x}'\mathbf{x} \rightarrow_p \boldsymbol{\pi}Q_z \boldsymbol{\pi} + \sigma_v^2\\
  	\widehat{\gamma}^2 &=& n^{-1}\mathbf{x}'Z(Z'Z)^{-1}Z'\mathbf{x} \rightarrow_p \boldsymbol{\pi}Q_z \boldsymbol{\pi}\\
  	\widehat{\sigma}_v^2 &=&  \widehat{\sigma}_x^2 - \widehat{\gamma}^2 \rightarrow_p \sigma_v^2
  \end{eqnarray*} 
so all that remains is to estimate the squared bias parameter $\tau^2$.

$\widehat{\tau} =  n^{-1/2} \mathbf{x}'\left(\mathbf{y} - \mathbf{x}\widetilde{\beta}_{2SLS}\right) \rightarrow_d N(\tau,V)$

$V =\sigma_\epsilon^2 \sigma_x^2(\sigma_x^2/\gamma^2 - 1) =  \sigma_\epsilon^2 \sigma_x^2(\sigma_v^2/\gamma^2)$
...
By Theorem \ref{pro:tau} and the Continuous Mapping Theorem, we have $\widehat{\tau}\widehat{\tau}' \rightarrow_d UU'$  where $U =\Psi M + \tau$. Since $E[M]=0$, $E[UU'] = \Psi \Omega \Psi' + \tau\tau'$. 
\end{proof}

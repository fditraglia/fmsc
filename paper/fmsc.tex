...
As we see from Theorems \ref{thm:consist} and \ref{thm:normality}, \emph{any} candidate GMM estimator $\widehat{\theta}_S$ is consistent for $\theta_0$ under local mis-specification. Unless $S$ excludes \emph{all} of the moment conditions contained in $h$, however, $\widehat{\theta}_S$ inherits an asymptotic bias from the mis-specification parameter $\tau$. The local mis-specification framework is useful precisely because it results in a limit distribution for $\widehat{\theta}_S$ with both a bias \emph{and} a variance. This captures in asymptotic form the bias-variance tradeoff that we see in finite sample simulations. In constrast, fixed mis-specification results in a \emph{degenerate} bias-variance tradeoff in the limit: scaling up by $\sqrt{n}$ to yield an asymptotic variance causes the bias component to \emph{diverge}. 


Any form of moment selection requires an identifying assumption. One approach, following \cite{Andrews1999}, is to assume that there exists a unique, maximal set of correctly specified moment conditions that identifies $\theta_0$. In the notation of the present paper this is equivalent to assuming that there exists an $S^*$ satisfying the following three conditions:
	\begin{enumerate}
		\item $|S^*|\geq r$
	 	\item $\Xi_{S^*} E[f(Z_{ni},\theta_0)]= 0$
	 	\item For any $S' \neq S^*$ such that $\Xi_{S'} E[f(Z_{ni},\theta')]= 0$ for some $\theta' \in \Theta$, $|S^*| > |S'|$.
	 \end{enumerate}
\cite{AndrewsLu} and \cite{HongPrestonShum} take the same basic approach to identification, with appropriate modifications to allow simultaneous model and moment selection. An advantage of this kind of identification assumption is that it allows consistent selection of $S^*$ without \emph{any} prior knowledge of which moment conditions are correct. In the notation of the present paper, this corresponds to having \emph{no} moment conditions in the $g$ block. As \citet[p.\ 254]{Hallbook} points out, however, the \cite{Andrews1999} identification assumption can easily fail, even in simple examples such as linear instrumental variables models. When it does, the selected GMM estimator may no longer be consistent for $\theta_0$. 

\todo[inline]{Explain the alternative identification assumption that I use here. Relate it to Liao and Cheng. Explain how it rests on the valid estimator being identified. Perhaps list this as an actual stand-alone assumption.}
...
For estimation based on $g$ alone to be possible, we must have $p \geq r$. With the exception of Section \ref{subsec:digress}, this assumption is maintained throughout. The \emph{full} estimator uses all moment functions, including the possibly invalid ones contained in $h$, namely $\widehat{\theta}_f = \argmin_{\theta \in \Theta}\; f_n(\theta)' \widetilde{W} \; f_n(\theta)$. For this estimator to be feasible, we must have $(p+q)\geq r$. 


\todo[inline]{Put a corollary with the distribution of the valid estimator and make sure to update the cross-refs later in the paper to reflect this.}

...
\begin{enumerate}[(a)]
	...
	\item For each $S\in \mathscr{S}$, $\widehat{\omega}_S \rightarrow_d\varphi(M|S)$, a function of $M$ (the normal random vector defined in Theorem \ref{thm:normality}) and constants only.
\end{enumerate}
\end{assump}
...
Under Assumption \ref{assump:weights} and the conditions of Theorem \ref{thm:normality},
	$$\sqrt{n}\left(\widehat{\mu} -  \mu_0\right) \rightarrow_{d}\Lambda(\tau) =  -\nabla_\theta\mu(\theta_0)'\left[\sum_{S \in \mathscr{S}} \varphi(M|S) K_S\Xi_S\right] M.$$
\end{cor}
...
\end{thm}
To connect these results to estimators post-selection, consider the weight function $\widehat{\omega}_S^{MSC} = \mathbf{1}\left\{\mbox{MSC}_n(S) = \min_{S'\in \mathscr{S}} \mbox{MSC}_n(S')\right\}$ where $\mbox{MSC}_n(S)$ is the value of some moment selection criterion evaluated at the sample observations $Z_{n1}\hdots, Z_{nn}$. Now suppose $\mbox{MSC}_n(S) \rightarrow_d\mbox{MSC}(M|S)$, a function of $M$ and constants only. Then, so long as the probability of ties, $\p\left\{\mbox{MSC}(M|S) = \mbox{MSC}(M|S') \right\}$, is zero for all $S\neq S'$, the continuous mapping theorem gives 
	$$\widehat{\omega}_S^{MSC} \rightarrow_d \mathbf{1}\left\{\mbox{MSC}(M|S) = \min_{S'\in \mathscr{S}} \mbox{MSC}(M|S')\right\}$$ 
...
\begin{equation}
\label{eq:omegahat}
	\widehat{\varphi}(\cdot|S) = \mathbf{1}\left\{\widehat{\mbox{FMSC}}(\cdot|S) = \min_{S'\in \mathscr{S}} \widehat{\mbox{FMSC}}(\cdot|S')\right\}
\end{equation}
...
\begin{enumerate}
	\item For each $\tau \in T(\widehat{\tau})$ 
		\begin{enumerate}[(i)]
			...
			\item Set $\Lambda_j(\tau) = -\nabla_\theta\mu(\widehat{\theta})'\left[\sum_{S \in \mathscr{S}} \widehat{\varphi}(M_j(\tau)|S) \widehat{K}_S\Xi_S\right] M_j(\tau)$

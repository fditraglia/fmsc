...
$$\Xi_1 = \left[\begin{array}{cc} I_{p} & 0_{p \times q}  \end{array}\right], \quad
    \Xi_2 = \left[ \begin{array}{cc}
              0_{q \times p}& I_{q}
            \end{array}\right]$$
so that $Z_1 = Z \Xi_1'$ and $Z_2 = Z \Xi_2'$.
...
Since $\Xi_S$ merely encodes a set of candidate moment conditions proposed by the researcher, the only remaining quantities from Equation \ref{eq:fmsc} are $\widehat{\boldsymbol{\tau}}$, $\widehat{\Psi}$ and $\widehat{\Omega}$. 
To proceed further, we first specialize Theorem \ref{thm:tau} as follows.
\begin{thm}
Let $\widehat{\boldsymbol{\tau}} = n^{-1/2} Z_2' \left( \mathbf{y} - X\widehat{\beta}_v\right)$. Under Assumption \ref{assump:chooseIV},
...
\end{eqnarray*}
where $M \sim N(0,\Omega)$, $\Xi_2$ is the moment selection matrix that corresponds to using only the instruments in $Z_2$, $Q$ is defined in $I_q$ is the $q\times q$ identity matrix.
\end{thm}
Using the preceding result, we construct the asymptotically unbiased estimator $\widehat{\tau}\widehat{\tau} - \widehat{\Psi}\widehat{\Omega} \widehat{\Psi}'$ of $\tau\tau$ from
	$$\widehat{\Psi} = \left[ \right]$$

\todo[inline]{Now just talk about $\Omega$ and specialize everything to the case of 2SLS, which we use in the empirical example and simulation study. Talk about centering / heteroskedasticity.}

...
Parts (a), (b) and (d) correspond to the local mis-specification assumption, part (c) is a set of moment restrictions, and (f) is simply the DGP.  Part (e) is the homoskedasticity assumption: an \emph{asymptotic} restriction on the joint distribution of the errors $v_{ni}$ and $\epsilon_{ni}$.   
...
\todo[inline]{OLS versus 2SLS example could be thought of as particular instance of the more general problem of instrument selection: if exogenous, $x$ is clearly its own ``best instrument.'' Now state the instrument selection problem in greater generality.}

Consider a linear instrumental variables (IV) model in a simple iid setting, namely
\begin{eqnarray}
...
\right]
\end{equation}
where $\mathbf{0}$ is a $q_1$-vector of zeros, and $\boldsymbol{\tau}$ is a $q_2$-vector of unknown constants. The following low-level conditions are sufficient for the asymptotic normality of $\widehat{\beta}_S$.
...
where
  ...
  with $Q_S = Q_Z \Xi_S'$ and the matrices $Q$ and $\Omega$ are defined in Assumption \ref{assump:chooseIV}.
\end{thm}

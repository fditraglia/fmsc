...
$$\mbox{AMSE(OLS)} = \frac{\tau^2}{\sigma_x^4} + \frac{\sigma_\epsilon^2}{\sigma_x^2},\quad \quad
 ...
 Rearranging, we see that the AMSE of the OLS estimator is strictly less than that of the 2SLS estimator whenever
 $$\tau^2  < \sigma_x^2 \sigma_\epsilon^2\left(\frac{\sigma_v^2}{\gamma^2}\right)$$
 To use this expression for moment selection we need to estimate the unknown parameters. Fortunately, the familiar estimators of $\sigma_x^2, \gamma^2$, and $\sigma_v^2$ remain consistent under Assumption \ref{assump:OLSvsIV} so we set
...
to estimate $\sigma_\epsilon^2$ below. All that remains is to estimate the squared bias parameter $\tau^2$. Specializing Theorem \ref{thm:tau} and Corollary \ref{cor:tautau} to the present example, we have the following.
\begin{thm}
	...
	$$\widehat{\tau}\rightarrow_d N(\tau,V)$$ 
	where $V =\sigma_\epsilon^2 \sigma_x^2(\sigma_x^2/\gamma^2 - 1) =  \sigma_\epsilon^2 \sigma_x^2(\sigma_v^2/\gamma^2)$.
\end{thm}
It follows that $\widehat{\tau}^2 -  \widehat{\sigma}_\epsilon^2\widehat{\sigma}_x^2 \left(\widehat{\sigma}_v^2/\widehat{\gamma}^2}\right)$ is an asymptotically unbiased estimator of $\tau^2$ and hence, substituting into the AMSE inequality from above and rearranging, the FMSC instructs us to choose OLS whenever
	$$\widehat{T}_{FMSC} = \frac{\widehat{\tau}^2 \widehat{\gamma}^2}{\widehat{\sigma}_v^2 \widehat{\sigma}_\epsilon^2 \widehat{\sigma}_x^2} < 2$$
The quantity $\widehat{T}_{FMSC}$ looks very much like a test statistic and indeed it can be viewed as such. Rewriting it as $\widehat{T}_{FMSC} = \widehat{\tau}^2/\widehat{V}$ where $\widehat{V} = \widehat{\sigma}_v^2 \widehat{\sigma}_\epsilon^2 \widehat{\sigma}_x^2/\widehat{\gamma}^2$ we see immediately that $\widehat{T}_{FMSC} \rightarrow_d \chi^2(1)$ by Theorem \ref{thm:tauOLSvsIV} and the continuous mapping theorem. This means that we can interpret the FMSC as a test of the null hypothesis $H_0\colon \tau = 0$ against the two-sided alternative with a critical value of $2$. Roughly speaking, this corresponds to a significance level of $\alpha \approx 0.16$.

\todo[inline]{Indeed, a little algebra shows that this test statistic is \emph{identical} to the Hausman test. Give outline of the algebra. Doesn't hold in all examples, but it does in this simple case. Give OLS the benefit of the doubt, but not free to choose critical value. Theoretical justification for empirical practice.}

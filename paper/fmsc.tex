...
Second, we know both from simulation studies \citep{Demetrescu} and analytical calculations \citep{LeebPoetscher2005} that selection dramatically changes the sampling distribution of our estimators.
Taken at face value, however, the asymptotics of consistent selection give the misleading impression that this effect can be ignored.
Lemma 1.1 of \citet[p.\ 168]{Poetscher1991}, for example, has been interpreted by some as evidence that traditional distribution theory continues to hold so long as we restrict ourselves to consistent selection criteria.
While his lemma does establish that the limit distributions of an estimator pre- and post-consistent selection are identical, \citet[pp.\ 179--180]{Poetscher1991} also makes it very clear that this result does not hold uniformly in the parameter space.
As such, a reliance on the lemma  ``only creates an illusion of conducting valid inference'' \citep[p.\ 22]{LeebPoetscher2005}.
Figure \ref{fig:consist} illustrates this problem via the simulation experiment from Section \ref{sec:fmscsim} by depicting the distributions of the valid and full estimators alongside those post-GMM-BIC and HQ.\footnote{These distributions are computed by kernel density estimation based on $10,000$ replications of the simulation described in Equations \ref{eq:secondstage} and \ref{eq:firststage}, each with a sample size of $500$ and $\gamma = 0.4$, $\rho= 0.2$.}
Because they are consistent criteria, and hence will exclude any invalid instruments in the limit, a na\"{i}ve reading of P\"{o}tscher's Lemma 1.1 would suggest that the post-selection distributions of GMM-BIC and HQ should be close to that of the valid estimator. 
This is emphatically not the case: the post-selection distributions are highly non-normal mixtures of the distributions of the valid and full estimators. 
\begin{figure}[htbp]
\begin{center}
...
\end{center}
\end{figure}
While Figure \ref{fig:consist} pertains to only one point in the parameter space, the problem is more general. As shown by Table \ref{tab:BICcov}, the empirical coverage probabilities of traditional $95\%$ confidence intervals are far lower than their nominal level over the vast majority of the parameter space.
The lack of uniformity is particularly striking: relatively small changes in the model parameters lead to a large deterioration in coverage.\footnote{Coverage probabilities for the GMM-HQ (available upon request) are similar.}
...
The point is not that these criteria fail to suffer from the non-uniformity problem. 
Indeed, simulation results for these are similar to GMM-BIC.
Instead it is that an asymptotic framework based on conservative selection and local asymptotics allows us to \emph{capture} this behavior when we derive limit distributions. 
Because FMSC, GMM-AIC and selection based on a downward $J$-test at a fixed significance level are conservative rather than consistent procedures, Lemma 1.1 of \citet{Poetscher1991} is inapplicable. 
Their behavior, however, is similar to that of the GMM-BIC.\footnote{Coverage probabilities for the FMSC appear below in Table \ref{tab:FMSCconf}.} 


In constrast, the asymptotics of conservative selection procedures under local mis-specification provide a far more accurate picture, and can be exploited to carry out valid inference post-selection.

In this section, we derive the asymptotic distribution of generic ``moment average'' estimators by extending the idea behind the frequentist model average estimators of \cite{HjortClaeskens}. 
...
We being by defining moment average estimators in general and considering some examples.
...
Hence, $\Lambda(\tau)$ is non-normal. The conditions of Assumption \ref{assump:weights} are fairly mild. Requiring that the weights sum to one ensures that $\widehat{\mu}$ is a consistent estimator of $\mu_0$ and leads to a simpler expression for the limit distribution. While somewhat less transparent, the second condition is satisfied by weighting schemes based on a number of familiar moment selection criteria. We see immediately from Corollary \ref{cor:FMSClimit}, for example, that the FMSC converges in distribution to a function of $\tau$, $M$ and consistently estimable constants only. The same as true for the $J$-test statistic, as we see from the following result. 
...
\todo[inline]{Work on this section from here down.}

To connect these results to estimators post-selection, consider the weight function 
...
Conveniently, the limiting behavior of $\widehat{\mu}$ follows \emph{almost immediately} from Theorem \ref{thm:normality}, which states that asymptotic distribution of $\widehat{\theta}_S$ depends only on $K_S$, $\tau$, and $M$. Because $K_S$ and $\tau$ are constants, $M$ governs the \emph{joint} limiting behavior of $\widehat{\theta}_S$, $S\in \mathscr{S}$. 
...
hus, it may be possible to achieve better performance by using smooth weights rather than discrete selection.

\todo[inline]{Merge in OLS versus IV material from other document.}

...
ordinary least squares (OLS) and two-stage least squares (2SLS) estimators. To keep the presentation simple, suppose that our goal is to estimate the effect $\beta$ of a single endogenous regressor $x$ on an outcome of interest $y$ in an iid, homoskedastic setting.\footnote{The results presented here are readily generalized to other settings.} 
...
The FMSC has a particularly convenient and transparent form in the OLS versus 2SLS example. Since the target parameter in this case is simply $\beta$, the FMSC amounts to comparing the AMSE of OLS to that of 2SLS. As an immediate consequence of Theorem \ref{thm:OLSvsIV}, we have
...
To estimate $\sigma_\epsilon^2$ we have two choices: we can either use the residuals from the OLS estimator or those from the 2SLS estimator. Under local mis-specification, both provide a consistent estimator of $\sigma_\epsilon^2$. We would expect the estimator based on the 2SLS residuals to be more robust, however, unless the instruments are quite weak. This is because the exogeneity of $x$, even though it disappears in the limit, is non-zero in finite samples. Thus, we use 
...
But how does this novel test statistic compare to something more familiar, say the Durbin-Hausman-Wu (DHW) test? It turns out that in this example, although not in others, the FMSC is \emph{numerically equivalent} to carrying out a DHW test with a critical value of 2. We can see this as follows. First, by Theorem \ref{thm:tauOLSvsIV}, 
...
\todo[inline]{Novel justification for the familiar DHW test to select between OLS and 2SLS, but the critical value should be 2, corresponding to $\alpha \approx 0.16$ rather than more familiar values such as 0.05 and 0.1. This also means that we have reason to believe that the FMSC is doing something reasonable. }

\subsection{FMSC for Instrument Selection Example}
\todo[inline]{In this example we're choosing overidentifying restrictions} As in the preceding example, the target parameter here is $\beta$.

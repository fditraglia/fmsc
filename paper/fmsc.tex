...
\todo[inline]{The following proof still needs cleanup.}

\begin{proof}[Proof of Theorem \ref{pro:sim}]
By Theorem \ref{thm:tau} and the Continuous Mapping Theorem we find that $\Delta_n(\widehat{\tau},\tau^*)\rightarrow_d \Delta(\tau,\tau^*)$ where
$$\Delta(\tau,\tau^*) =  (\Psi M + \tau - \tau^*)' (\Psi\Omega\Psi')^{-1} \left(\Psi M + \tau - \tau^*\right)$$
follows a \emph{non-central} $\chi^2$ distribution with $q$ degrees of freedom and non-centrality parameter $\tau - \tau^*$.


$\widehat{a}_{min}(\widehat{\tau})$ to $a_{min}(\Psi M + \tau)$ 

$\widehat{b}_{max}(\widehat{\tau})$ to $b_{max}(\Psi M + \tau)$ 

$a_{min}(\Psi M + \tau)=\min \left\{a(\tau)\colon \tau \in T(\delta)\right\}$

$b_{max}(\Psi M + \tau)=\max \left\{b(\tau)\colon \tau \in T(\delta)\right\}$

$\mathscr{T}(\Psi M + \tau,\delta) = \left\{\tau^* \colon  \Delta(\Psi M +\tau, \tau^*) \leq \chi^2_q(\delta) \right\}$ 

Corollary \ref{cor:momentavg}, $\sqrt{n}(\widehat{\mu} - \mu_0) \rightarrow_d\Lambda(\tau)$ \emph{jointly} with the above.
...
By the Continuous Mapping Theorem,
	$$P\left[  \widehat{a}_{min}(\widehat{\tau}) \leq \sqrt{n}\left( \widehat{\mu}-\mu_0\right)\leq  \widehat{b}_{max}(\widehat{\tau}) \right] \rightarrow P \left[ a_{min}(\Psi M) \leq \Lambda(\tau) \leq b_{max}(\Psi M)\right]$$

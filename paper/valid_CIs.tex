...
Tables \ref{tab:OLSvsIVsim_cover_naiveFMSC} and \ref{tab:chooseIVsim_cover_naiveFMSC} show the problem of ignoring moment selection.
Naive CI can be extremely poor or close to correct, depending on parameter values.
This is precisely the uniformity problem: even in large samples small changes in parameters can lead to large changes in coverage.
...
\centering
	...
	\end{subtable}
	\caption{Coverage probabilities of nominal 90\% CIs for the OLS versus 2SLS example. Based on 10,000 simulation draws from the DGP given in Equations \ref{eq:OLSvsIVDGP1}--\ref{eq:OLSvsIVDGP3}}
\end{table}
...
\centering
	...
	\end{subtable}
	\caption{Coverage probabilities of nominal $90\%$ CIs for the choosing instrumental variables example. Based on 10,000 simulation draws from the DGP given in Equations \ref{eq:chooseIVDGP1}--\ref{eq:chooseIVDGP3}.}
\end{table}

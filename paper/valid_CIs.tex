...
\todo[inline]{
Question of whether these intervals can be improved. Yes and no. 
No because there are impossibility results here: Leeb etc. 
This is the cost of trying to be uniform: you have to be conservative.
Yes because \emph{any} combination of $\alpha$ and $\delta$ that sum to 0.1 will yield an interval with asymptotic coverage of at least 0.9.
Could try to optimize these to get the shortest possible interval, as McCloskey.
Primary obstacle is computational: these intervals are already computationally intensive.
Adding another layer to the problem only makes it worse.

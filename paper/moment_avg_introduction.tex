...
\label{cor:FMSClimit}
	...
	\end{eqnarray*}
\end{cor}
The preceding corollary implies that the FMSC is a ``conservative'' rather than ``consistent'' selection procedure.
...
%This fact is readily apparent from the results of the simulation study from Section \ref{sec:OLSvsIVsim}, presented in Figure \ref{fig:OLSvsIV_RMSEvsDHW}.
%Consistent selection between OLS and 2SLS estimators via a DHW test would require taking the significance level, $\alpha$, to zero as the sample size increases. 
%Smaller critical values, however, lead to a deterioration in worst-case performance as we see from the figure.
...
Rather, it is that conservative criteria can be studied in a framework that allows us to capture the effects of selection on subsequent inference.
...
Second, the FMSC and related methods carry out selection over estimators that \emph{may be mis-specified}.
Indeed, the FMSC explicitly \emph{sets out} to use mis-specified moment conditions when doing so yields a favorable bias-variance tradeoff.
Even if our goal were to eliminate all mis-specified moment conditions, for example by using a consistent criterion such as the GMM-BIC of \cite{Andrews1999}, in practice we would not always be successful.
Whenever there is a positive probability of selecting a mis-specified estimator, either on purpose or by mistake, a standard confidence interval that assumes all moment conditions to be correctly specified will be incorrectly centered.
...
To this end, the present section derives the asymptotic distribution of generic ``moment average'' estimators by extending the idea underlying the frequentist model average estimators of \cite{HjortClaeskens}. 
...
Because the resulting limit distributions are analytically intractable, we propose a two-step, simulation-based procedure for constructing confidence intervals with \emph{uniform} asymptotic coverage guarantees. 

...
Rather, it is that conservative criteria can be studied in a framework that allows us to capture the effects of selection on inference.
The intuition is as follows.
Each candidate estimator has a sampling distribution that is approximately normal by the central limit theorem.
The post-selection estimator, however, is a \emph{randomly-weighted average} of the candidate estimators.
Accordingly, its distribution is approximately a \emph{mixture} of normals in finite samples.
Under the asymptotics of consistent selection, a single specification is selected with probability approaching one in the limit so the mixture becomes degenerate.

\todo[inline]{No way for consistent selection asymptotics to represent the non-normality. In contrast, conservative selection is random even in the limit, allowing us to give the non-normality encountered in finite-samples an asymptotic representation.}

To this end, we derive the asymptotic distribution of generic ``moment average'' estimators by extending the idea underlying the frequentist model average estimators of \cite{HjortClaeskens}. 

...
rm(Baseline, Climate, Europe, Openness, MalfalSq, RuleSq)

#-----------------------------------------------
#      FIT 2SLS FOR EACH INSTRUMENT SET 
#-----------------------------------------------
instrument.formulas <- lapply(instrument.sets, function(x){
  paste("~", paste(x, collapse = " + "))
})

tsls.fits <- lapply(instrument.sets,
                    function(first.stage){
                      tsls(model, as.formula(first.stage), CGdata)
                    })

rm(instrument.formulas, model)

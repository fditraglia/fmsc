...
#------------------------------------------------------#
#---------END FUNCTION: FMSC.2SLS.conf.naive-----------#
#------------------------------------------------------#
...
#Naive corrected interval
FMSC.malfal.var2 <- FMSC.2SLS.conf.naive(X, y, Z.valid, Z.additional.var2, additional.instruments.var2, mu.malfal, nabla.malfal)



#-------------------------------------------------------------------------#
#----Now, try to get the actual two-stage confidence procedure to work----#
#-------------------------------------------------------------------------#

#First, define a function that calculates the FMSC for each submodel and returns the quantities needed to create the confidence intervals.
...
first.step <- FMSC.2SLS.return.Ks(X, y, Z.valid, Z.additional.var2, additional.instruments.var2, mu.malfal, nabla.malfal)
...
Lambda.conf <- 0.95
bottom <- (1 - Lambda.conf)/2
top <- 1 - (1 - Lambda.conf)/2


#Calculate as many things as possible outside of the objective function!
Psi.transpose <- t(Psi)
Sigma <- Psi %*% Omega %*% t(Psi)
Sigma.vec <- c(Sigma)


target.project <- function(K.S){
  
  crossprod(nabla.mu.valid, K.S)
  
}#END target.project

neg <- function(x){return(-x)}

outer.terms <- lapply(K.list, target.project)
multiplier.matrices <- lapply(outer.terms, neg)
null.limit.bias.matrix <- matrix(0, nrow = (q1+q2), ncol = (q1+q2))
...
lambda.conf <- function(tau.test, which.tail){
  ...
  for(j in 1:B){
    ...
    #Lower right block of limit version of bias.matrix
    #Psi.Mj <- Psi %*% M.j  
    #Psi.Mj.Mj.Psi <- Psi.Mj %*% t(Psi.Mj)
    #lower.block <- Psi.Mj.Mj.Psi - Sigma
  
    lower.block <- Psi %*% {M.j %*% t(M.j) - Omega} %*% Psi.transpose
      
    #Construct limit version of bias.matrix
    limit.bias.matrix <- null.limit.bias.matrix 
    limit.bias.matrix[(q1+1):(q1+q2), (q1+1):(q1+q2)] <- lower.block
    ...
    #Now we can get the limit version of the FMSC for each of the submodels by creating a function to apply over K.S.list
    FMSC.limit <- function(outer.term.S){
    
        result <- outer.term.S %*% limit.inner.matrix %*% t(outer.term.S)
        
        return(result)  
    }#END FMSC.limit
    
  
  FMSC.limit.values <- lapply(outer.terms, FMSC.limit)
  ...
  #This is a confidence interval for sqrt(n)(mu.hat - mu.true)
  lower.lambda <- quantile(Lambda, bottom)
  upper.lambda <- quantile(Lambda, top)
...
Rprof()
lower.lambda <- lambda.conf(tau.hat, 'lower')
Rprof(NULL)
summaryRprof()



#A (possibly) faster version using matrix operations
#Define MANY helper functions since we need to vectorize everything
FMSC.limit <- function(Psi.Mj){
  
  lower.block <- tcrossprod(Psi.Mj) - Sigma
  limit.bias.matrix <- null.limit.bias.matrix 
  limit.bias.matrix[(q1+1):(q1+q2), (q1+1):(q1+q2)] <- lower.block
  
  inner.matrix <- limit.bias.matrix + Omega
  out <- lapply(outer.terms, function(X) X %*% tcrossprod(inner.matrix, X))

}#END FMSC.limit


sim.multiplier <- function(FMSC.list){
  
  return(multiplier.matrices[[which.min(FMSC.list)]])
...
lambda.conf.matrix <- function(tau, which.tail){

  M.mean <- c(rep(0, q1), tau)
    
  #Generate Samples from M
  M.sims <- t(rmvnorm(B, M.mean, Omega))
  
  #Vectorized version of lower right block of limit bias matrix
  Psi.M <- Psi %*% M.sims
  
  FMSC.results <- apply(Psi.M, 2, FMSC.limit)
  
  sim.multipliers <- lapply(FMSC.results, sim.multiplier)

  M.list <- lapply(apply(M.sims, 2, list), unlist)
  Lambda <- mapply(inner.product,  sim.multipliers, M.list)
...
}#END lambda.conf.matrix




#So how does this vectorized version perform?
Rprof()
lower.lambda <- lambda.conf.matrix(tau.hat, 'lower')
Rprof(NULL)
summaryRprof()
...
lambda.upper.neg.penalty <- function(tau.test){
  ...
  #This is a confidence interval for sqrt(n)(mu.hat - mu.true)
  neg.upper.lambda <- -1 * quantile(Lambda, top)

  out <- neg.upper.lambda + penalty
  return(out)
    
}#END lambda.upper.neg.penalty
...
lambda.lower.penalty.matrix <- function(tau.test){
...
}#END lambda.lower.penalty.matrix    
    
    

lambda.upper.neg.penalty.matrix <- function(tau.test){
...
}#END lambda.upper.neg.penalty.matrix    
...
controlDE <- list(checkWinner = TRUE)

results.lower <- DEoptim(fn = lambda.lower.penalty.matrix, lower = lower.tau, upper = upper.tau, control = controlDE)$optim$bestval

results.upper <- -1 * DEoptim(fn = lambda.upper.neg.penalty.matrix, lower = lower.tau, upper = upper.tau, control = controlDE)$optim$bestval

lower.lambda <- results.lower
upper.lambda <- results.upper
